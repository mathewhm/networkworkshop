...
First, generate a simple network from an ER model (we specify number of nodes and edges)
...
m <- 1000 
# Sample ER 
ER_graph <- sample_gnm(n, m, directed = FALSE, loops = FALSE)

# igraph allows us to easily plot these graph objects
plot(ER_graph, vertex.size = 10) # lots of options here, for now we keep it simple
...
igraph allows us to easily obtain statistics on our graph such as centrality measures, degree distribution, etc
```{r}
# density of graph
...
l <- layout_with_dh(ER_graph)

plot(ER_graph, vertex.label.color="black", 
     vertex.label.cex=.5,vertex.label.dist=0.2, vertex.size =15, edge.curved=0.5,layout=l)
```
...
From the model above, we get:

$-1.45*$change in edges$+ 0.005*$change in triangles


Edge that creates no new triangles, the conditional log-odds is: $-1.45$ (which yields probability $exp(-1.45)/(1+exp(-1.45))= 0.189$

One edge that creates one triangle: $(-1.45 + 0.005 \times 1) = -1.445$ with corresponding probability $0.191$

One edge that creates two triangles: $(-1.45 +00.005\times 2) = -1.440$ with corresponding probability $0.192$

where the probability is interpretted as the probability that two nodes are connected given we add an edge
...
truth <- make_clusters(karate, V(karate)$Faction)

# plot by different centrality measures
deg_centrality  <- centr_degree(karate, mode="in", normalized=TRUE)$res

clos_centrality <- centr_clo(karate, mode="all", normalized=T)$res

eig_centrality  <- centr_eigen(karate, directed=T, normalized=T)$vector

bet_centrality  <- centr_betw(karate, directed=T, normalized=T)$res
```
...
Exercise: Calculate misclustered nodes and compare to the edge betweenness method

## Another Method: Label propagation algorithm
...
gg2 <- ggplot(ah_covar_df, aes(as.factor(female))) + 
  geom_histogram(stat ="count")+ xlab("Female")
...
```{r, cache = TRUE, message=FALSE, warning = FALSE, fig.height = 8, fig.width = 12, fig.align = "center", echo = FALSE}
####
diag(ah_net)<-0
...
amen_basic <- ame(Y = ah_net, 
             ...
             R = 0, nscan = 10000, burn = 500, 
             model = "frn", plot = FALSE)

summary(amen_basic)
hist(c(amen_basic$EZ), main = "Histogram of Expected Z", xlab = "Expected Z value")
```

***
### Question
How well does this model appear to fit our data?

# Making dyadic covariates
Lets try to make some improvements! Consider creating dyadic covariates to indicate whether or not two people share particular attributes. 
...
amen_R2 <- ame(Y = ah_net, 
             ...
             Xdyad = dyadic_covar,
             dcor = TRUE, rvar = TRUE, cvar = TRUE,
             R = 2, nscan = 10000, burn = 500,
             model = "frn", plot = FALSE)

hist(c(amen_R2$EZ), main = "Histogram of Expected Z", xlab = "Expected Z value")
pred_net_R2 <- 1*(amen_R2$EZ>0)
max(rowSums(pred_net_R2))
...
amen_R3 <- ame(Y = ah_net, 
             ...
             Xdyad = dyadic_covar,
             dcor = TRUE, rvar = TRUE, cvar = TRUE,
             R = 3, nscan = 10000, burn = 500, 
             model = "frn", plot = FALSE)

hist(c(amen_R3$EZ), main = "Histogram of Expected Z", xlab = "Expected Z value")
pred_net_R3 <- 1*(amen_R3$EZ>0)
max(rowSums(pred_net_R3))
...
amen_R4 <- ame(Y = ah_net, 
             ...
             Xdyad = dyadic_covar,
             dcor = TRUE, rvar = TRUE, cvar = TRUE,
             R = 4, nscan = 10000, burn = 500, 
             model = "frn", plot = FALSE)

hist(c(amen_R4$EZ), main = "Histogram of Expected Z", xlab = "Expected Z value")
pred_net_R4 <- 1*(amen_R4$EZ>0)
max(rowSums(pred_net_R4))
...
modelnames <- c("basic", "dyad", "R1", "R2","R3") 
...
```{r, eval = FALSE}
n = # number of nodes you want, pick something kind of small < 1000
p = # vector with 3 entries that sum up to 1 (probability of belonging to each community)
B = matrix(c(), nrow = 3)# 3 by 3 symmetric matrix that indicates probability of connection between and within communities (pick what you want!)
blockSizes = n*p

sampleSBM <- sample_sbm(n = n, pref.matrix =B , block.sizes =blockSizes , directed = FALSE, loops = FALSE) # sample a network
adj_mat_SBM <- as_adj(sampleSBM) # turn graph object into matrix

trueLabels <- c(rep(1, n*p[1]), rep(2, n*p[2]), rep(3, n*p[3])) # get true labels
```

 Pick a clustering method and see how well it does at recovering the true labels
...
First, make our adjacency matrix 
```{r}
...
```{r}
# get top 9 houses
house_subset_labels     <- names(sort(table(union_characters$house), decreasing = TRUE)[1:9]) 
...
subset_characters$color <- ifelse(is.na(subset_characters$color), "blue", subset_characters$color)
...
We can try some more clustering methods! Consider a spectral clustering algorithm
```{r}
...
Things are not looking so good... lets visually see what communities are being selected 
