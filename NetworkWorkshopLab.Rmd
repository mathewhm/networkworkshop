...
truth   <- make_clusters(karate, V(karate)$Faction) #you can create a 'cluster' type object to be associated with the graph
```
...
$-1.84*$change in edges$0.028*$change in triangles


Edge that creates no new triangles, the conditional log-odds is: $-1.84$ (which yields probability $exp(-1.84)/(1+exp(-1.84))= 0.137$)

One edge that creates one triangle: $(-1.84  + 0.028 \times 1) = -1.812$ with corresponding probability $0.140$

One edge that creates two triangles: $(-1.84 +  0.028\times 2) = -1.784$ with corresponding probability $0.144$
...
# visualizing our adj matrix (function to plot adj mat)
...
matrixPlotter(adj_mat_SBM,n=300)
